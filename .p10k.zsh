# Generated by Powerlevel10k configuration wizard on 2020-03-17 at 13:32 CET.
...
#   taskwarrior             # taskwarrior task count (https://taskwarrior.org/)
  ...
  function prompt_my_cpu_temp() {
	...
	temp_total=0
        ...
        do
                temp_total=$( echo "$temp_total + $t" | bc )
        done
        temp=$( echo "scale=1; $temp_total / $temp_count" | bc )
        if (( $temp >= 65 )); then
                p10k segment -s HOT -f red -i $'\uf490' -t "${temp}°C"
        elif (( $temp >= 50 )); then
                p10k segment -s WARM -f yellow -i $'\uf2c7' -t "${temp}°C"
        else
                p10k segment -s WARM -f green -i $'\uf2dc' -t "${temp}°C"
        fi
  }
#  function net_iface() {
